...
export class TypeOrmClinicRepository implements IClinicRepository {
    ...
    constructor(
        ...
        @InjectRepository(SpecialityModel) private readonly specialityModel: Repository<SpecialityModel>
) { }
    ...
    async findForSpeciality(speciality: string): Promise<ClinicModel[]> {;
        ...
        const specialityFound = await this.specialityModel.findOne({where: {name: speciality}});
    ...
    async findForName(name: string): Promise<ClinicModel[]> {
        return await this.clinicRepository.find({where: {name}, relations: ['specialities']});
    }
